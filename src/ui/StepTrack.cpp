...
void StepTrack::stepButtonClicked(StepButton *button,
  ...
  if (auto index = getStepButtonIndex(button); index >= 0) {
    ...
    listeners.call([this, globalIndex](Listener &l) {
...
void StepTrack::handleStepSelection(StepButton *button,
                                    const juce::ModifierKeys &mods) {
  ...
  if (mods.isShiftDown() && lastSelectedStepIndex >= 0) {
    ...
    auto it = std::find(selectedStepIndices.begin(), selectedStepIndices.end(),
                        globalIndex);
...
int StepTrack::getStepButtonIndex(StepButton *button) const {
  auto it =
      std::find_if(stepButtons.begin(), stepButtons.end(),
                   [button](const auto &ptr) { return ptr.get() == button; });
...
void StepTrack::addListener(Listener *listener) { listeners.add(listener); }
